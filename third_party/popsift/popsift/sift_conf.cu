...
void Config::setDescMode(const std::string& text) {
    if (text == "loop")
        ...
        POP_FATAL(
            "specified descriptor extraction mode must be one of loop, grid or "
            "igrid");
}
...
void Config::setScalingMode(ScalingMode mode) { _scaling_mode = mode; }
...
#define COMPARE(a) (this->a != other.a)
    if (COMPARE(octaves) || COMPARE(levels) || COMPARE(sigma) ||
        COMPARE(_edge_limit) || COMPARE(_threshold) ||
        COMPARE(_upscale_factor) || COMPARE(_scaling_mode) ||
        COMPARE(_max_extrema) || COMPARE(_gauss_mode) || COMPARE(_sift_mode) ||
        COMPARE(_assume_initial_blur) || COMPARE(_initial_blur) ||
        COMPARE(_normalization_mode) || COMPARE(_normalization_multiplier))
        return false;
    return true;
}
