...
#ifdef DEPTH_ALIGNED_ACCESSES
    const uint8_t* u8p = reinterpret_cast<const uint8_t*>(data);
    return ((uint32_t)u8p[3] << 24) | ((uint32_t)u8p[2] << 16) |
           ((uint32_t)u8p[1] << 8) | u8p[0];
#else
...
void QuantizeDepthImage(int width, int height, const uint16_t* depth,
                        std::vector<uint16_t>& quantized);
void DequantizeDepthImage(int width, int height, const uint16_t* quantized,
                          std::span<uint16_t> depth);

//------------------------------------------------------------------------------
// Zstd

void ZstdCompress(const std::vector<uint8_t>& uncompressed,
                  std::vector<uint8_t>& compressed);

bool ZstdDecompress(const uint8_t* compressed_data, int compressed_bytes,
                    int uncompressed_bytes, std::vector<uint8_t>& uncompressed);
...
void Pack12(const std::vector<uint16_t>& data, std::vector<uint8_t>& packed);

void Unpack12(const std::vector<uint8_t>& packed, std::vector<uint16_t>& data);
...
class DepthCompressor {
   public:
    ...
    DepthResult Compress(int width, int height,
                         const uint16_t* unquantized_depth,
                         std::vector<uint8_t>& compressed, bool keyframe);
    ...
    DepthResult Decompress(const std::vector<uint8_t>& compressed, int width,
                           int height, std::vector<uint16_t>& depth_out);

    // (mark liu)
    // adding this interface so I don't have to
    // have an std::vector to decompress
    DepthResult Decompress(const uint8_t* compressed,
                           const unsigned int compressed_bytes, int width,
                           int height, std::vector<uint16_t>& depth_out);

    DepthResult Decompress(const uint8_t* compressed,
                           const unsigned int compressed_bytes, int width,
                           int height, std::span<uint16_t> depth_out);
   ...
   protected:
    ...
    void CompressImage(int width, int height, const uint16_t* depth,
                       const uint16_t* prev_depth);
    bool DecompressImage(int width, int height, uint16_t* depth,
                         const uint16_t* prev_depth);

    void WriteCompressedFile(int width, int height, bool keyframe,
                             std::vector<uint8_t>& compressed);

    void EncodeZeroes(int width, int height, const uint16_t* depth);

    // This writes the whole QuantizedDepth image with 0 or 1
    // before the block decoding starts.
    void DecodeZeroes(int width, int height, uint16_t* depth);
};
