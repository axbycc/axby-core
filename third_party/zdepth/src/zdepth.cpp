...
DepthResult DepthCompressor::Decompress(
		...
		const unsigned int compressed_bytes,
		int width,
		int height,
		std::span<uint16_t> depth_out) {

    if (depth_out.size() != size_t(width*height)) return DepthResult::BadDimensions;
		
    if (compressed_bytes < kDepthHeaderBytes) {
...
DepthResult DepthCompressor::Decompress(
		...
		return Decompress(compressed, compressed_bytes, width, height, std::span<uint16_t>(depth_out));
}
...
DepthResult DepthCompressor::Decompress(
    ...
    std::vector<uint16_t>& depth_out)
{
		return Decompress(compressed.data(), compressed.size(), width, height, depth_out);
