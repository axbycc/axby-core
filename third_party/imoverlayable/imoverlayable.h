...
struct Overlay {
    ...
    float scale;
		...
		void line(float x1, float y1, float x2, float y2, uint32_t color, float thickness);
		...
		void polyline(std::span<const float> polyline, std::span<const uint32_t> colors, float thickness);
		void line_list(std::span<const float> line_list, std::span<const uint32_t> colors, float thickness);
...
Overlay image(void* texture_id, float data_width, float data_height, float display_width = 0);
